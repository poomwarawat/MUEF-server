...
router.post("/send-score", (req, res) => {
  ...
  if (mode === "INH") {
    const sql = `INSERT INTO test_result (codeId, INH) VALUES ('${codeId}',${parseInt(
      score
    )})`;
    con.query(sql, (err, result) => {
      if (err) throw err;
  ...
  if (mode === "SHF") {
    const sql = `UPDATE test_result SET SHF=${parseInt(
      score
    )} WHERE codeId='${codeId}'`;
    con.query(sql, (err, result) => {
      if (err) throw err;
  ...
  if (mode === "EC") {
    const sql = `UPDATE test_result SET EC=${parseInt(
      score
    )} WHERE codeId='${codeId}'`;
    con.query(sql, (err, result) => {
      if (err) throw err;
  ...
  if (mode === "WM") {
    const sql = `UPDATE test_result SET WM=${parseInt(
      score
    )} WHERE codeId='${codeId}'`;
    con.query(sql, (err, result) => {
      if (err) throw err;
  ...
  if (mode === "PO") {
    const sql = `UPDATE test_result SET PO=${parseInt(
      score
    )} WHERE codeId='${codeId}'`;
    con.query(sql, (err, result) => {
      if (err) throw err;
...
router.post("/send-score-102", (req, res) => {
  console.log(req.body);
  const { score, mode, codeId } = req.body;
  if (mode === "INH") {
    const sql = `INSERT INTO test_result2 (codeId, INH) VALUES ('${codeId}',${parseInt(
      score
    )})`;
    con.query(sql, (err, result) => {
      if (err) throw err;
  ...
  if (mode === "SHF") {
    const sql = `UPDATE test_result2 SET SHF=${parseInt(
      score
    )} WHERE codeId='${codeId}'`;
    con.query(sql, (err, result) => {
      if (err) throw err;
  ...
  if (mode === "EC") {
    const sql = `UPDATE test_result2 SET EC=${parseInt(
      score
    )} WHERE codeId='${codeId}'`;
    con.query(sql, (err, result) => {
      if (err) throw err;
  ...
  if (mode === "WM") {
    const sql = `UPDATE test_result2 SET WM=${parseInt(
      score
    )} WHERE codeId='${codeId}'`;
    con.query(sql, (err, result) => {
      if (err) throw err;
  ...
  if (mode === "PO") {
    const sql = `UPDATE test_result2 SET PO=${parseInt(
      score
    )} WHERE codeId='${codeId}'`;
    con.query(sql, (err, result) => {
      if (err) throw err;
...
router.get("/get-score-state-102/:id", (req, res) => {
  console.log(req.params.id);
  const data = {
    INH: false,
    SHF: false,
  ...
  const sql = `SELECT * FROM test_result2 WHERE codeId='${req.params.id}'`;
  con.query(sql, (err, result) => {
    if (err) throw err;
    if (result) {
      for (var i in result) {
...
router.get("/get-state-result-102/:id", (req, res) => {
  console.log(req.params.id);
  const sql = `SELECT * FROM test_result2 WHERE codeId='${req.params.id}'`;
  con.query(sql, (err, result) => {
    for (var i in result) {
  ...
  con.query(sql, (err, result) => {
    ...
    if (result) {
      for (var i in result) {
        ...
        const birthdayDate = `${birthdayTime.getUTCDate() + 1}/${
          birthdayTime.getUTCMonth() + 1
        }/${birthdayTime.getUTCFullYear()}`;
        const created = `${create.getUTCDate()}/${
          create.getUTCMonth() + 1
        }/${create.getUTCFullYear()}`;
...
router.get("/get-result-102/:id", (req, res) => {
  const Data = [
    { INH: 0 },
    { SHF: 0 },
    { EC: 0 },
  ...
  con.query(sql, (err, result) => {
    ...
    if (total !== 0) {
      const sql_result = `UPDATE test_result2 SET total=${total} WHERE codeId='${req.params.id}'`;
      con.query(sql_result, (err, result) => {
        if (err) throw err;
        if (result) {
          Data[5].total = total;
...
router.get("/check-total-score-102/:id", (req, res) => {
  const sql = `SELECT total FROM test_result2 WHERE codeId='${req.params.id}'`;
  console.log(req.params.id);
  con.query(sql, (err, result) => {
    if (err) throw err;
    if (result) {
